...
| `RESEND_FROM` | (선택) Resend 발신 주소 | `onboarding@resend.dev` 또는 인증 도메인 |
...
| `RESEND_FROM` | Resend 발신 주소 | 테스트: `onboarding@resend.dev` (가입 이메일로만 발송). 실서비스: 도메인 인증 후 `noreply@yourdomain.com` 등 |
...
| 변수명 | 설명 | 예시 값 |
|--------|------|---------|
| `VITE_API_URL` | 백엔드 API 기준 URL (끝에 `/` 없이) | `https://salestoolproejct.onrender.com` |
...
| Render (Web Service) | `DATABASE_URL`, `RESEND_API_KEY`, `RESEND_FROM` | Internal URL 사용 |
| Vercel | `VITE_API_URL` | 백엔드 URL, Redeploy 필요 |
