...
3. **마이그레이션만** 필요하면 **Actions** → **「Migrate database (Render)」** → **Run workflow** (시드 없이 `npm run migrate` 만 실행). 워크플로: `.github/workflows/migrate-database.yml`
4. 시드·관리자까지 한 번에 넣을 때는 **「Seed oper database (Render)」** → **Run workflow** — 최초에는 `run_seed_admin`·`run_migrate`·`run_seed` 를 조합(이미 시드했으면 시드만 끔). 워크플로: `.github/workflows/seed-oper-database.yml`

**한 번만 설정:** GitHub 시크릿 **`DATABASE_URL`** 에 운영 DB URL을 넣어 두면, 이후에는 Actions만으로 마이그레이션을 반복할 수 있다. URL·비밀번호는 **레포에 커밋하지 않는다.** 로컬은 `server/.env.example` 을 참고해 `server/.env` 를 만든다.
